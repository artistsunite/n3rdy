    ...
    <div className={`border-l-2 pl-3 ${borderColor(p.status)}`}>
      <button className="w-full flex items-start justify-between gap-2 py-2 text-left hover:bg-white/2 rounded-r-lg pr-2 transition-colors" onClick={() => setOpen(v => !v)}>
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <span className="text-sm text-white font-medium">{p.target}</span>
          <span className="text-xs text-white/50">· {p.timeframe}</span>
          <StatusChip status={p.status} />
          {p.outcome?.validatedBy && (
            <span className="text-xs text-white/30">({p.outcome.validatedBy === 'AUTO' ? 'auto' : 'you'})</span>
          )}
        </div>
        <div className="flex items-center gap-2 flex-shrink-0 mt-0.5">
          <span className="text-xs text-white/50">{new Date(p.expiresAt).toLocaleDateString()}</span>
          {open ? <ChevronUp size={13} className="text-white/50" /> : <ChevronDown size={13} className="text-white/50" />}
        </div>
      </button>
      {open && (
        <div className="pb-3 space-y-2">
          <p className="text-xs text-white/50 leading-relaxed">{p.reasoning}</p>
          {p.outcome?.actualDirection && (
            <p className="text-xs text-white/50">Actual: <span className="text-white">{p.outcome.actualDirection}</span></p>
          )}
          {p.outcome?.userNotes && (
            <p className="text-xs text-white/50">Your notes: <span className="text-white">{p.outcome.userNotes}</span></p>
          )}
          {p.feedback?.filter(f => f.answer).map(f => (
            <div key={f.id} className="text-xs space-y-0.5">
              <p className="text-white/50 italic">"{f.question}"</p>
              <p className="text-white">→ {f.answer}</p>
            </div>
          ))}
        </div>
      )}
  ...
  if (resolved.length === 0) {
    return (
      <div className="text-center py-10">
        <p className="text-sm text-white/50">No resolved predictions yet.</p>
        <p className="text-xs text-white/30 mt-1">Predictions will appear here once their timeframe expires.</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-3">
      <div className="flex gap-2 flex-wrap">
        {FILTERS.map(f => (
          <button
            ...
            className={`text-xs px-3 py-1 rounded-full border transition-colors ${filter === f.value ? 'bg-n3-primary/20 border-n3-primary/40 text-n3-primary' : 'bg-transparent border-white/10 text-white/50 hover:border-white/30'}`}
          >
            {f.label}
          </button>
        ))}
      ...
      <div className="space-y-1">
        ...
        {filtered.length === 0 && <p className="text-sm text-white/50 text-center py-4">No {filter.toLowerCase()} predictions.</p>}
      </div>
    </div>
  );
}
